...
def show_all(query):
    ...
    products = query['TAG_NAME'].unique()
...
def show_data_product(df2, product, today, daily_credits, monthly_credits, yearly_credits, description):
    """_summary_
    ...
    col1, col2 = container_a.columns([0.8, 1], gap="large")
    
    n_tables = len(df2['OBJ_NAME'].unique())

    col1.markdown(f'<p style="color:#29b5e8; font-family:Source Sans Pro, sans serif; font-size: 25px;"><b>{title}: </b></p> ', unsafe_allow_html=True)
    col2.markdown(f'<p style="color:#29b5e8; font-family:Source Sans Pro, sans serif; font-size: 25px;"><b>CRÉDITOS COBRADOS EM {today}:</b></p>', unsafe_allow_html=True)

    with col1:
        col1.write(f'Owner: {df2["OWNER"].unique()[0]}')
        col1.write(f'Quantidade de objetos: {n_tables}')
        col1.write(f'Tempo de atualização: {df2["REFRESH_VALUE"].unique()[0]}')
        df2['END_TIME'] = pd.to_datetime(df2['END_TIME'])
        max_date = df2['END_TIME'].max()
        max_date = pd.to_datetime(max_date).strftime('%Y/%m/%d')
        col1.write(f'Última data de atualização: {max_date}')
    ...
    containerb = col2.container(border=False)
    with containerb:
        st.markdown(
        """
        <style>
    ...
    st.markdown('<p style="color:#3d3d3c; font-family:Source Sans Pro, sans serif; font-size: 25px;"><b>Monitoramento de Créditos Cobrados</b></p>', unsafe_allow_html=True)
    ...
    col1.markdown('<p style="color:#3d3d3c; font-family:Source Sans Pro, sans serif; font-size: 20px;"><b>Data de monitoramento e processos: </b></p>', unsafe_allow_html=True)
    col1_2.write('Os processos ao serem selecionados, mostrarão o gasto diário e do mês da data selecionada.')
    ...
    col2_2.write('A data de monitoramento ao ser definida, mostrará o gasto diário e do mês da data selecionada.')
