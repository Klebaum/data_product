...
def plot_credit_billed_month(df, date):
    ...
    adjust_d = adjust_d[['END_TIME', 'forecast_transform', 'forecast_ml', 'results', 'Total']]
    #st.write(adjust_d)
...
def plot_credit_billed_day(df, date):
    ...
    fig = px.bar(adjust_d, x='QUERY_TAG', y='CREDITS_USED_PER_USER_APROX', title=f'Créditos gastos em {date_to_filter}')
...
def show_all_products(query):
    ...
    with col1:
        ...
        with col1.expander('Detalhes das informações do produto'):
            df = query[['TAG_VALUE', 'OBJ_NAME', 'REFRESH_VALUE']]
...
#   return rows, cols, colsN
    ...
    if len(procces) != 0:
        ...
        if selected_product in list_products:
            ...
            container2.subheader('Gastos do Data Product: ')
